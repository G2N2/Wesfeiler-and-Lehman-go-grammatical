...
from libs.layer_exhaust_gnn import G2N2Layer
from torch_geometric.nn import (global_add_pool,global_mean_pool, global_max_pool)
import libs.readout_exhaust_gnn as ro
...
class exhaust_GNN(nn.Module):
    def __init__(self,  node_input_dim, edge_input_dim, output_dim, device,
                 ...
                 readout_type  = "sum" ,level = "graph",relu = True):
        
        super(exhaust_GNN, self).__init__()
        ...
        if self.level == "graph" or level == "node":
            self.fc = nn.ModuleList( [torch.nn.Linear(2*self.nodes_dim[-1]+2*self.edges_dim[-1],self.final_neuron[0])])
    ...
    def forward(self,data):
        ...
        if self.relu:
            for i,l in enumerate(self.conv):
                x,v,C=(l(x,v, edge_index, C,batch_node))
            ...
            for l in self.conv:
                x,v,C=(l(x,v, edge_index, C,batch_node))
        x = self.readout(x,v,C,data.batch,data.batch_edge,data.node_batch_edge,identite)
